...
# (9/7/22): NEED TO FIX LIST LIMITATIONS AND UNDERSTAND NUMPY ARRAYS! (DONE: 9/8/22)

# (9/8/22): GOAL: GATHER TEAM'S SEASON HISTORY DATA (DONE: 9/12/22)
    # This will also need to incorporate scraping team names and putting into array.

# (9/10/22): Use CSS selectors to get table data for each different webpage. (DONE: 9/12/22)
    # Need to fix pandas to csv issue, get team names into array, and find out how to get their
    # season data as well. (FIXED)

# (9/13/22): Need to fix CSV layout issues including @'s, /'s, missing features, etc. (DONE: 9/26/22)
    # Use string replace, string split, and conditionals to solve. (FIXED)
    # Fix game time at some point as well. (FIXED)

# (9/26/22): Need to get team names across all divisions of college lacrosse. (DONE: 10/3/22)

# (10/3/22): Need to create function that uses team names to find their season history. (DONE: 10/4/22)
    # Minor detail but need to fix the time issue with strings. (FIXED)

# (10/4/22): Need to run function to make sure it works appropriately and with no issues. (DONE: 10/13/22)
    # Had error with W&L time having '/' (FIXED)
    # Error when a team doesn't have a minimum of 10 seasons. (FIXED)
    # Error with Maryland being the second choice when typing in 'Maryland' (FIXED)
    # Error after 36 teams have been recorded (FIXED)

# (10/13/22): THESE ARE THE FIXES THAT MUST BE DONE BEFORE FINAL RUN!
    # Change sleep to wait (TBD)
    # /n string in Neutral games needs to be fixed (FIXED)
    # Add '0' to Penalties feature if empty (FIXED)

# (10/24/22): Get final data (D1, D2, D3). (DONE: 11/13/22)
    # Changed main XPATH to CSS and reduced redundant code (Fixed)
    # Created 0 placements for Goals, Assists, Points, Shots, SOG. (FIXED)
    # Created a multiple suspended games check (FIXED)
        #  - might want to go over if errors come.
    # Created a 'Unable to find driver" check (FIXED)
        # This is only a temporary solution but I've only encountered one team with this.
    # Replace most instances of split with replace where there is string manipulation. (FIXED)
        # This will simply the code above and clean it up.   
    # Wheeling Error - team that doesn't have 2020 season messes up all of the code because of SeasonNo. (FIXED)
        # There could be more teams that follow this.
    # Fixed score string issues and instead used replace rather than split (FIXED)
    # Cazenovia error --> Missing 1 season causes a big bug. (FIXED)
    # Having runtime errors again in regard to unable to find exceptions. (FIXED?)
    # Spalding Error - clicking to view season history gives "Unable to find player" (FIXED)

# (11/15/22): Gather Roster size, Head coach, Location of game (DONE: 12/11/22)
    # Added 'general' vs. 'other' option in main function, since the traversal to team websites are the same. (FIXED)
    # Error with cancelled games, need to design mechanism to filter these out. (FIXED)
    # LIU error - New data being added on website? (FIXED)
    # SacredHeart error - head coach doesn't exist on one season. (FIXED)

    # ['Brown', 'Dartmouth', 'LIU', 'Alabama Huntsville', 'Lenoir-Rhyne'] - All have a statement in 2021, other data might be +-1. Change by hand. (LOOK OVER AT THIS AFTER RUNNING)
    # ['Birmington So.', 'Rhodes', 'SUNY Geneseo', 'SUNY New Paltz', 'William Peace'] - Change by hand. (LOOK OVER AT THIS AFTER RUNNING) (DONE)



# (DATE): Gather more data for the dataset.
    # Easy - Roster size, # Seasons Played, Head Coach, Stadium Capacity,
        # Playoffs Last Year (NCAA), Coaches Seasons w/Team, Coaches Record.
    # Medium - Weather, Elevation, Win Streak/Loss Streak 